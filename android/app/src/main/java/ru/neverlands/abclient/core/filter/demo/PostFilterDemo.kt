package ru.neverlands.abclient.core.filter.demo
...
/**
 * Демонстрация работы PostFilter системы
 * Показывает как фильтры модифицируют ответы от neverlands.ru
 */
@Singleton
class PostFilterDemo @Inject constructor(
    ...
    /**
     * Демонстрирует фильтрацию различных типов контента
     */
    fun demonstrateFiltering() {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            // 4. Демонстрация настроек пользователя
    ...
    private suspend fun demonstrateJavaScriptFilters() {
        ...
        // Тестируем фильтр информации о персонаже
    ...
    private suspend fun demonstratePhpFilters() {
        ...
        // Тестируем главную игровую страницу
        ...
        if (mainPhpResponse.isSuccessful) {
            ...
            // Проверяем модификации времени действий
            val hasWtimeModification = content.contains("Выполняется действие...")
            Log.d(TAG, "Wtime modification applied: $hasWtimeModification")
            
            // Проверяем парсинг боевых данных
        ...
        // Тестируем торговлю (если включена)
    ...
    private suspend fun demonstrateHtmlFilters() {
        ...
        // Тестируем информацию о персонаже
        ...
        if (pinfoResponse.isSuccessful) {
            ...
            // Проверяем парсинг данных персонажа
    ...
    /**
     * Демонстрация влияния настроек пользователя
     */
    private fun demonstrateUserSettings() {
        ...
        // Эти настройки влияют на поведение фильтров
    ...
    /**
     * Демонстрирует обработку специфических игровых ситуаций
     */
    suspend fun demonstrateGameSituations() {
        ...
        if (userProfile.fishAuto) {
            ...
            if (fishingResponse.isSuccessful) {
                ...
                val hasFishData = content.contains("рыба")
        ...
        if (userProfile.torgActive) {
            ...
            if (tradeResponse.isSuccessful) {
                val content = tradeResponse.body?.string() ?: ""
                val isTradeOffer = content.contains("Купить вещь у")
    ...
    /**
     * Тестирует производительность фильтров
     */
