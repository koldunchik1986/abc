...
/**
 * HTTP фильтр для модификации ответов от neverlands.ru
 * Полный аналог ABClient\PostFilter\Filter.cs из Windows версии
 */
@Singleton
class HttpFilter @Inject constructor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        // Пропускаем не-neverlands.ru запросы
     ...
     * Аналог PreProcess() из Windows версии
     */
     ...
     * Аналог Process() из Windows версии
     */
    ...
    private fun processJavaScript(url: String, data: ByteArray, html: String): ByteArray? {
        return when {
            ...
            url.endsWith("/ch/ch_list.js", ignoreCase = true) -> {
    ...
    /**
     * Строит редирект страницу
     * Аналог BuildRedirect() из Windows версии
     */
